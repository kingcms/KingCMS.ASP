			...
			if not rs.eof and not rs.bof then
				...
				for i=0 to length'开始循环列表
					...
					if cstr(listid)<>cstr(data(1,i)) then
						listid=data(1,i)
						set irs=conn.execute("select listname,listpath from king__movie_list where listid="&listid&";")
					...
					king.value "commentid",encode(r_path&"|"&data(0,i))'传递评论参数
					king.value "params",encode(paramhtm(data(0,i),datalist(2,j),data(13,i)))
					if len(sql_field)>0 then
						fields=split(sql_field,",")
